...
| `KINRESOLVE_TURNSTILE_SECRET_KEY` | Required only when native applications are enabled; distinct server-only Cloudflare Turnstile siteverify secret. A verified widget token unlocks the standard intake rate lanes; token-free, rejected, or unverifiable submissions use the strict lanes instead of being refused |
