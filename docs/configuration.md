...
| `KINRESOLVE_TURNSTILE_SECRET_KEY` | Required only when native applications are enabled; distinct server-only Cloudflare Turnstile siteverify secret. A verified widget token unlocks the standard intake rate lanes; token-free, rejected, or unverifiable submissions use the strict lanes instead of being refused. The public demo cell reuses this secret for its session-start challenge |
| `KINRESOLVE_DEMO_TURNSTILE_MODE` | Public demo session-start Turnstile ladder: `off` (default), `shadow` (verify and log without changing admission), or `required` (reject unverified human starts with the stateless family/challenge fallbacks). Authorized canaries always bypass the challenge |
| `NEXT_PUBLIC_KINRESOLVE_DEMO_TURNSTILE_SITE_KEY` | Public Turnstile widget key for the demo start form; required whenever the demo Turnstile mode is `shadow` or `required` |
| `NEXT_PUBLIC_SENTRY_DSN` | Optional public Sentry ingest identifier for hosted error tracking. Events are aggressively scrubbed (no headers, cookies, query strings, bodies, or user identity), tracing and replay stay disabled, and self-hosted builds without a DSN load no Sentry at all |
