...
| `DATABASE_URL` / migration database credential | Runtime or protected migration/recovery jobs | Runtime and migration credentials must remain distinct. For the public demo, update Vercel's Sensitive `DATABASE_URL` and the protected GitHub `demo-production` secret `PUBLIC_DEMO_RUNTIME_DATABASE_URL` to the same new bounded runtime URL in one holding-page maintenance window. Rotate against the exact database identity and verified TLS endpoint; prove the grant receipt's runtime-role digest matches protected candidate health and run application smoke. Never give a runtime role DDL/fence-control authority or a cleanup environment source access. |
