...
describeIfDatabase("direct private integration uploads", () => {
  ...
  it("enforces plain GEDCOM bytes before tickets and after a hosted capability flip", async () => {
    ...
    const stagedBeforeFlip = await stageDirectIntegrationUpload(
      connection.id,
      { fileName: "queued-before-flip.zip", contentType: "application/zip", size: 8 },
      firstOptions
    );
    backend.set(issuedKey!, Buffer.from([0x50, 0x4b, 0x03, 0x04, 0, 0, 0, 0]), "application/zip", "etag-before-flip");
    backend.stream.mockClear();
    vi.mocked(ticketIssuer.issue).mockClear();
